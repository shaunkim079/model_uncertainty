...
if(length(commandArgs(TRUE))!=0){
  ...
  if(length(arguments)>7) SD2<-as.numeric(arguments[[8]])
  if(length(arguments)>8) theta_file<-as.character(arguments[[9]])
  if(length(arguments)>9) seed<-as.numeric(arguments[[10]])
  if(length(arguments)>10) output_dir<-as.character(arguments[[11]])
  if(length(arguments)>11) ITER<-as.numeric(arguments[[12]])
  if(length(arguments)>12) stop_update_covariance<-as.numeric(arguments[[13]])
  if(length(arguments)>13) restart_number<-as.numeric(arguments[[14]])
}
...
if(!is.loaded("routing_gr4j")){
  if(Sys.info()["sysname"]=="Linux"){
    dyn.load("packages/gr4j_with_routing/src/gr4j.so")
  } else {
    dyn.load("packages/gr4j_with_routing/src/gr4j.dll")
  }
}
...
logprior_fun<-trial_log_prior4_gr4jwithrouting_allinitstates3
loglikelihood_fun<-log_likelihood_trial4_gr4jwithrouting_allinitstates3
...
for(i in start_iter:ITER){
  ...
  #   state_normaliser<-10^theta[1:i,length_ts+length_ts+2]
