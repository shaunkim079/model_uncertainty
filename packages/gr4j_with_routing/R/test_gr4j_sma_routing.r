...
source("gr4j_with_routing/R/gr4j_sma_routing.r")
if(!is.loaded("routing_gr4j")){
  if(Sys.info()["sysname"]=="Linux"){
    dyn.load("gr4j_with_routing/src/gr4j.so")
  } else {
    dyn.load("gr4j_with_routing/src/gr4j.dll")
  }
}
...
timer<-system.time({
  for(i in 1:1000){
    tt1<-gr4j.run(param=param, initial_state_S=actual_initial_state, initial_state_R=100, 
                  state_error=actual_state_error, input=true_input, return_state=F)
  }
})
