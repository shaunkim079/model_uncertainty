...
# only sample state error and input error using discharge error sd sd and input error sd sd, using distribution of sample variance #############################
# https://en.wikipedia.org/wiki/Variance#Distribution_of_the_sample_variance
log_likelihood_trial4_gr4jwithrouting_allinitstates5<-function(data,params){
  #library(Brobdingnag,lib.loc="packages")
  # Cochran's theorem shows that s^2 follows a scaled chi-squared distribution
  logd_sample_variance<-function(variance,N,expected_variance){
    frac<-variance/expected_variance
...
trial_log_prior4_gr4jwithrouting_allinitstates5<-function(params,min,max,data,likelihood=NULL){
  ...
  combined_log_prior<-log_prior_initial_state+state_normaliser_prior+log_prior_state_errors_normalised+log_prior_initial_state_R+sum(log_prior_input_errors) #+log_prior_state_sds #+log_prior_error_input_variance+log_prior_error_discharge_variance+sum(log_prior_input_errors)+sum(log_prior_state_errors)
  #if(is.infinite(combined_log_prior)) browser()
...
# only sample state error and input error using discharge error sd sd and input error sd sd, using distribution of sample variance #############################
# https://en.wikipedia.org/wiki/Variance#Distribution_of_the_sample_variance
log_likelihood_trial4_gr4jwithoutrouting_allinitstates5<-function(data,params){
  #library(Brobdingnag,lib.loc="packages")
  # Cochran's theorem shows that s^2 follows a scaled chi-squared distribution
  logd_sample_variance<-function(variance,N,expected_variance){
    frac<-variance/expected_variance
...
trial_log_prior4_gr4jwithoutrouting_allinitstates5<-function(params,min,max,data,likelihood=NULL){
  ...
  combined_log_prior<-log_prior_initial_state+state_normaliser_prior+log_prior_state_errors_normalised+sum(log_prior_input_errors) #+log_prior_state_sds #+log_prior_error_input_variance+log_prior_error_discharge_variance+sum(log_prior_input_errors)+sum(log_prior_state_errors)
  #if(is.infinite(combined_log_prior)) browser()
  # combined_log_prior<-0
...
# only sample state error and input error using discharge error sd sd and input error sd sd, using distribution of sample variance #############################
# https://en.wikipedia.org/wiki/Variance#Distribution_of_the_sample_variance
log_likelihood_trial4_gr4jwithoutrouting_allinitstates5_without_scale<-function(data,params){
  #library(Brobdingnag,lib.loc="packages")
  # Cochran's theorem shows that s^2 follows a scaled chi-squared distribution
  logd_sample_variance<-function(variance,N,expected_variance){
    frac<-variance/expected_variance
  ...
  length_ts<-length(data$input)
  state_normaliser<-10^(params[length_ts+length_ts+1]*10)
  state_normaliser<-10^(log10(data$normalisers[2])/10*10)
  params_unnorm<-inv.normalise(params[1:(length_ts*2)],c(data$normalisers[1],rep(state_normaliser,length_ts-1),data$normalisers[(length_ts+1):(length_ts*2)]))
  initial_state<-params_unnorm[1]
  state_error<-params_unnorm[2:length_ts]
  input_error<-params_unnorm[(length_ts+1):(length_ts+length_ts)]
...
trial_log_prior4_gr4jwithoutrouting_allinitstates5_without_scale<-function(params,min,max,data,likelihood=NULL){
  #   inv.normalise<-function(x,factor){
  #     x/factor
  #   }
  # browser()
...
# only sample state error and input error using discharge error sd sd and input error sd sd, using distribution of sample variance #############################
# https://en.wikipedia.org/wiki/Variance#Distribution_of_the_sample_variance
log_likelihood_trial4_gr4jwithrouting_allinitstates3_hs_play_bates_routing_error_adjusted_v5.8<-function(data,params){
  #library(Brobdingnag,lib.loc="packages")
  # Cochran's theorem shows that s^2 follows a scaled chi-squared distribution
  logd_sample_variance<-function(variance,N,expected_variance){
    frac<-variance/expected_variance
...
# only sample state error and input error using discharge error sd sd and input error sd sd, using distribution of sample variance #############################
# https://en.wikipedia.org/wiki/Variance#Distribution_of_the_sample_variance
log_likelihood_trial4_gr4jwithrouting_allinitstates3_hs_play_bates_routing_error_adjusted_v5.8_no_init<-function(data,params){
  #library(Brobdingnag,lib.loc="packages")
  # Cochran's theorem shows that s^2 follows a scaled chi-squared distribution
  logd_sample_variance<-function(variance,N,expected_variance){
    frac<-variance/expected_variance
  ...
  if(!is.na(model_run[1])){
    ...
    if(all(!is.na(sim_flow_trans))){
      ...
      # I'm adding the prior for discharge here for convenience
      discharge_error_min<--sqrt(population_flow_variance_trans_agg)*1e6
      discharge_error_max<-sqrt(population_flow_variance_trans_agg)*1e6
      error_discharge_prior<-sum(dunif(residual_trans,min=discharge_error_min,max=discharge_error_max,log=T))
...
trial_log_prior4_gr4jwithrouting_allinitstates3_hs_play_bates_routing_error_adjusted_v5.8_no_init<-function(params,min,max,data,likelihood=NULL){
  #   inv.normalise<-function(x,factor){
  #     x/factor
  #   }
  sd_zero_mean<-function(x) sqrt(mean(x^2))
